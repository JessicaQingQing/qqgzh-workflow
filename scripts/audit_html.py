...
class Scanner(HTMLParser):
    def __init__(self): super().__init__(); self.sources=[]; self.takeaway=False; self.connect=False; self.cta=False; self.has_article=False
    def handle_starttag(self,tag,attrs):
        ...
        if "cta" in classes: self.cta=True
        if tag in ("article","body","h1"): self.has_article=True
def inspect(html):
    parser=Scanner(); parser.feed(html)
    return {
        "html_present":bool(parser.has_article and re.search(r"<h1\b",html,re.I)),
        "forbidden_css":not bool(BAD_CSS.search(html) or BAD_TAG.search(html)),
        "image_hosts":not bool(parser.sources) or all(src.startswith("https://mmbiz.qpic.cn/") for src in parser.sources),
        "takeaway":parser.takeaway,
        "connect_optional":True,
        "sensitive_content":not bool(SENSITIVE.search(html)),
        "fact_traceable":(parser.has_article or parser.takeaway),
        "third_party_anonymized":True,
    }
def main():
