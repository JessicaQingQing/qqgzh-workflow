...
REQUIRED_V2={"identity":("name","professional_background","key_experiences"),"content":("directions",)}
...
def validate(profile):
    ...
    if sv not in (1,2): errors.append(f"schema_version must be 1 or 2, got {sv}")
    if sv == 1:
        for section,fields in {"brand":("name","tagline","voice","avoid"),"audience":("primary","needs"),"visual":("accent_color","image_style","approved_image_hosts"),"connection":("primary_channel","account_name","invitation"),"monetization":("offers","claims_policy"),"publishing":("mode","requires_explicit_confirmation")}.items():
        ...
        for section,fields in REQUIRED_V2.items():
            item=profile.get(section)
            if not isinstance(item,dict): errors.append(f"missing object: {section}"); continue
            for field in fields:
                val=item.get(field)
                if isinstance(val,list) and len(val)==0: errors.append(f"empty list: {section}.{field}")
                elif val in (None,"",{}): errors.append(f"missing value: {section}.{field}")
        if profile.get("publishing",{}).get("requires_explicit_confirmation") is not True: errors.append("publishing.requires_explicit_confirmation must be true")
