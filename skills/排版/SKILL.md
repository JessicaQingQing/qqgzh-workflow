---
name: 排版
description: 把文章草稿 + 商业连接方案 → 微信兼容 HTML。只用微信不会吞的 CSS，手机优先。这是公众号渠道适配器，不是所有内容输出的必经步骤。
---
...
这是**公众号渠道适配器**——只有用户想把内容发布到微信公众号时才用。职业资产卡、方向总结、选题清单等过程产出不需要排版。

## 吃进去什么

- 文章草稿（纯文本或 Markdown）
- 商业连接方案（`商业连接` 出来的，可能是"这篇不加"）
- 用户 Profile ——品牌语气、强调色、连接 CTA（如果启用）、商业话术
...
一个自包含的 `.html` 文件，结构按需组合：

1. `<h1>` 大标题
2. `<p class="lead">` 开头钩子段落
3. 封面图（可选——没图就直接跳过）
4. 正文，含 `<h2>` 章节标题和段落
5. **`.takeaway` 金句区** ——读者看完能带走的一句话
6. 商业落点（可选——商业连接方案没声明就跳过 `.cta` 区块）
7. `— END —` 结束标记
8. **`.connect` 连接区** ——仅当商业连接方案声明需要时加入
9. 作者简介一行（可选）
...
- 图片只从 Profile 里批准的主机名加载（默认只有 `https://mmbiz.qpic.cn/`）。
